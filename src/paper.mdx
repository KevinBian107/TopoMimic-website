...
### One clip, every transition

Zooming all the way in: a single rodent clip rich enough to span four distinct behaviors (Immobile, Walking, Left Turn, Rear). Following the same logic the paper uses, we keep one cluster minimum per behavior label and connect every minimum pair with its DM-path. The left panel is a t-SNE of the clip's intention features, where each frame is colored by its behavior and a star marks each cluster's inverse-density minimum. As the video plays, every DM-path on the t-SNE grows simultaneously, and each segment of the line is colored by the cluster the point underneath it belongs to — so the line itself reads as a behavior-to-behavior gradient. The rows on the right play the rodent traversing each path in real time; the body's tint shifts continuously from the source cluster's color to the destination cluster's color, the same gradient idea applied to the actual movement.
